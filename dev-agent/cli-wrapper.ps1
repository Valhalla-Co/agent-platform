...
Write-Host "Dev Agent CLI wrapper: requestId=$requestId intent=$intent repoPath=$repoPath"

# Prepare branch workspace (optional placeholder)
$dest = Join-Path -Path $repoPath -ChildPath "branches/dev-agent/$branchName"
...
try {
    if (Test-Path '.\mvnw.cmd') {
        Write-Host "Running mvnw -pl :agent-core -DskipTests clean package (log: $buildLog)"
        .\mvnw.cmd -pl :agent-core -DskipTests clean package *> $buildLog
        ...
        if ($exitCode -ne 0) {
            ...
            $jar = Get-ChildItem -Path '.\agent-core\target' -Filter '*.jar' -ErrorAction SilentlyContinue | Select-Object -First 1
