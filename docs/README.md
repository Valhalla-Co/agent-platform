# Documentation Index

## Setup & Configuration
- [Java Config](JAVA_CONFIG.md) - Configuração do Java JDK
- [Setup Guide](SETUP_GUIDE.md) - Guia completo de configuração
- [Quick Reference](QUICK_REFERENCE.md) - Referência rápida
- [Início Rápido](INICIO_RAPIDO.md) - Guia de início rápido (PT-BR)

## Architecture & Restructuring
- [Restructure Plan](RESTRUCTURE_PLAN.md) - Plano detalhado da reestruturação
- [Restructure Complete](RESTRUCTURE_COMPLETE.md) - Resumo da reestruturação concluída
- [Class Mapping](CLASS_MAPPING.md) - Mapeamento de classes antes/depois

## Odin Agent
- [Odin README](ODIN_README.md) - Documentação principal do Odin Agent
- [Odin Brain Guide](ODIN_BRAIN_GUIDE.md) - Guia completo do sistema de brain e lessons

## Status & History
- [Init Status](INIT_STATUS.md) - Status inicial do projeto
- [Build Success](BUILD_SUCCESS.md) - Registro de build bem-sucedido
- [Verbosity Reduction](VERBOSITY_REDUCTION.md) - Documentação da redução de verbosidade

---

## Quick Links

### Getting Started
1. [Java Config](JAVA_CONFIG.md) - Configure o JDK primeiro
2. [Quick Reference](QUICK_REFERENCE.md) - Comandos essenciais
3. [Odin README](ODIN_README.md) - Como usar o Odin Agent

### Advanced Topics
- [Odin Brain Guide](ODIN_BRAIN_GUIDE.md) - Sistema de aprendizado e repositórios
- [Restructure Complete](RESTRUCTURE_COMPLETE.md) - Entenda a arquitetura atual

---

**Main README**: [../README.md](../README.md)
